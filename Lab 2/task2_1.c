/*
Task: Write a program processes.c, and let the parent process produce two child processes.
...
int main() {
    ...
    else { //Code for parent after first child
        wait(&waitStatus[0]); // Wait for first child to terminate
        int child2 = fork();
        if(child2 == 0){ // Code for second child
            printf("\nI am second child, my pid is: %d\n", getpid());
            wait(&waitStatus[1]);
        }
        else { // Code for parent after second child
            wait(&waitStatus[1]);
        }
    }
    return 0;
